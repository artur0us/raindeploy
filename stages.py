...
class Stages:
    ...
    def build_golang_project(stage_name: str, stage_details: dict, env_credentials: dict) -> Union[str, bool]:
        ...
        try:
            ...
            if Data.WORK_MODE == DEBUG_MODE:
                print_logs(proc_out, proc_err)
            if ("error" in str(proc_out).lower()) or ("error" in str(proc_err).lower()) or ("undefined:" in str(proc_out).lower()) or ("undefined:" in str(proc_err).lower()) or ("cannot use" in str(proc_out).lower()) or ("cannot use" in str(proc_err).lower()):
